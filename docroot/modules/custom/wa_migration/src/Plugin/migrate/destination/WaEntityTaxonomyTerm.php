 ...
 */
#[MigrateDestination('wa_term:taxonomy_term')]
class WaEntityTaxonomyTerm extends EntityContentBase {
  ...
  protected function getEntity(Row $row, array $old_destination_id_values): EntityInterface {
    ...
    if ($mail = $row->getSourceProperty('name')) {
      if ($terms = $this->storage->loadByProperties([
        'name' => $mail,
        'vid' => $this->getBundle($row),
      ])) {
        return reset($terms);
      }
    }
