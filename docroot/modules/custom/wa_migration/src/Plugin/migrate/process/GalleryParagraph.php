 ...
 * process:
 *   bar:
 *     plugin: gallery_paragraph
 *     source: foo
 * @endcode
 *
 * @MigrateProcessPlugin(id = "gallery_paragraph")
 */
final class GalleryParagraph extends ProcessPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property): ?ParagraphInterface {
    if ($value) {
      if ($media = $this->entityTypeManager->getStorage('media')->load($value)) {
        ...
        $parent = $this->entityTypeManager->getStorage('paragraph')->create([
          'type' => 'gallery_item',
        ]);
        $parent->set('field_image_paragraph', $paragraph);
        $parent->enforceIsNew();
