...
final class AssetToOrangeDam extends ProcessPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property): mixed {
    if ($value) {
      ...
      // For speed, we'll assume all images have had their asset bank id set.
      if ($destination_property == 'field_media_dam_image') {
        $result = $this->api->search(['query' => 'image-id-(asset-bank).Image-ID-(Asset-Bank):' . $value]);
      }
      else {
