...
/**
 * Provides a 'Donation Menu Footer' condition.
 *
 * @Condition(
 *   id = "donation_menu_footer",
 *   label = @Translation("Donation Menu Footer"),
 *   context_definitions = {
 *     "block" = @ContextDefinition("entity:block", label = @Translation("Block"), required = FALSE),
 *   }
 * )
 */
class DonationMenuFooter extends ConditionPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function evaluate(): bool {
    ...
    if (isset($donation_page_menus_visibility[0]['value'])) {
      if ($donation_page_menus_visibility[0]['value'] === 'footer' || $donation_page_menus_visibility[0]['value'] === 'both') {
        return TRUE;
      }
    }
    return FALSE;
