...
trait PcaAddressElementTrait {
   ...
   *   Info array.
   */
  public function buildElementGetInfo(): array {
    return [
      '#pca_fields' => [],
      '#pca_options' => [],
   ...
   *   The updated element.
   */
  public static function processPcaAddress(array &$element, FormStateInterface $form_state, array &$complete_form): array {
  ...
  private static function addAddressLookupField(array &$element): void {
    ...
    $lookup_failed = TRUE;
    $element['#allow_manual_input_on_fail'] = TRUE;
    if ($element['#show_address_fields'] !== TRUE && ($element['#allow_manual_input'] === TRUE || ($element['#allow_manual_input_on_fail'] && $lookup_failed))) {
  ...
  private static function preparePcaOptions(array &$element): void {
    ...
    if (isset($element['#pca_options'])) {
      // Before we merge the options, check for an API key value.
